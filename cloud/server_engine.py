...
def handle_request():
    ...
    try:
        ...
        if task == 'ask_ai':
            ...
            try:
                response = ai_client.models.generate_content(model='gemini-2.5-flash', contents=f"{system_gate}\n\nERROR:\n{error_msg}\n\nCONTEXT:\n{context_snippet}")
        ...
        # ==========================================
        # 📦 SUB_DOCS HANDLER (Writes shortcut files like variables.sty to temp_dir)
        # ==========================================
...
def get_key():
    try:
        return jsonify({"status": "success", "geminiApiKey": GEMINI_API_KEY or ""})
    except:
        return jsonify({"status": "error", "geminiApiKey": ""})
...
def run_goo_server():
    ...
    # ==========================================
    # 🚀 NGROK TUNNEL (with region fix + retry loop)
    # ==========================================
    ...
    if not public_url:
        print("❌ Could not establish ngrok tunnel after 4 attempts.")
        return

    # ==========================================
    # 📝 UPDATE COMM FILE WITH NEW URL
    # ==========================================
    reg = {}
    ...
    <div style='padding: 20px; border: 2px solid #2e7d32; border-radius: 10px; background-color: #f1f8e9; font-family: sans-serif;'>
        ...
        <p><b>Project:</b> <code>{os.path.basename(os.path.dirname(os.path.dirname(COMM_FILE)))}</code></p>
